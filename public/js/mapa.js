...
form_add.addEventListener('submit', function(event) {
	...
	var points = drawLayer.getSource().getFeatures()[0].getGeometry().getCoordinates();
	if(points.length > 1){		
		const formData = {};
		const nodos = [];
		let i = 1;
		for (var c in points) {
			...
			const nodoExistente = nodos.find((n) => n.latitud === nodo.latitud && n.longitud === nodo.longitud);
			if (nodoExistente) {
			console.log('Punto duplicado. No se agrega a la matriz.');
			continue; // Saltar a la siguiente iteración del bucle
			}
			nodo['lugarCercano'] = obtenerLugarCercano(nodo['latitud'], nodo['longitud']);
			nodos.push(nodo);
		}
		drawLayer.getSource().clear();
